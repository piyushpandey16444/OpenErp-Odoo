...
class MrpWorkorder(models.Model):
    ...
    workcenter_id = fields.Many2one(
        ...
        states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    # workcenter_ids = fields.One2many('mrp.workcenter', 'id', string='Work Centers')
    ...
    # Gaurav 5/5/20 added and commented lot serial field to compute sequence
    # final_lot_id = fields.Many2one(
    #     'stock.production.lot', 'Lot/Serial Number', domain="[('product_id', '=', product_id)]",
    #     states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    final_lot_id = fields.Many2one(
            'stock.production.lot', 'Lot/Serial Number', domain="[('product_id', '=', product_id)]",
            states={'done': [('readonly', True)], 'cancel': [('readonly', True)]}, compute='get_final_lot_id')
    # Gaurav end
    ...
    # Gaurav 5/5/20 added compute function to show in final lot id(lot/serial no)
    def get_final_lot_id(self):
        spl = self.env['stock.production.lot']
        #Himanshu mrp  16-12-2020 added for loop due to singleton error when pressing the done on work order
        for val in spl:
            spl_search = val.search([('product_id', '=', self.product_id.id)])
            if spl_search:
                self.final_lot_id = spl_search[0]
        #End Himanshu
    #         Gaurav end
