...
class ProductTemplate(models.Model):
    ...
    # Aman 19/08/2020 Commented this function because now accounts are fetched from products and journals.
    # @api.multi
...
class ProductProduct(models.Model):
    ...
    def _convert_prepared_anglosaxon_line(self, line, partner):
        # Aman 29/11/2020 Added condition to get correct debit and credit values for vendor bill, customer invoice,
        # sales return, purchase return when journal entries are created
        res = {
        ...
        if line['type'] == 'in_invoice_dest' or line['type'] == 'out_refund_dest':
            res.update({'credit': line['price'] < 0 and -line['price']})
        elif line['type'] == 'in_invoice' or line['type'] == 'out_refund':
            res.update({'debit': line['price']})
        elif line['type'] == 'out_invoice_dest' or line['type'] == 'in_refund_dest':
            res.update({'debit': line['price']})
        elif line['type'] == 'out_invoice' or line['type'] == 'in_refund':
            if line['price'] < 0:
                res.update({'credit': line['price'] < 0 and -line['price']})
            else:
                res.update({'credit': -line['price']})
        else:
            res.update({
