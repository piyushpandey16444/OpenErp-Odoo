...
class AccountChartTemplate(models.Model):
    ...
    def _prepare_all_journals(self, acc_template_ref, company, journals_dict=None):
        def _get_default_account(journal_vals, type='debit'):
            ...
            # Aman 27/11/2020 Added condition to check credit_note and debit_note and add accounts to them
            if journal['type'] == 'sale' or journal['type'] == 'credit_note':
                default_account = acc_template_ref.get(self.property_account_income_categ_id.id)
            elif journal['type'] == 'purchase' or journal['type'] == 'debit_note':
        ...
        # Aman 27/11/2020 Added new last 4 journals
        journals = [{'name': _('Customer Invoices'), 'type': 'sale', 'code': _('INV'), 'favorite': True, 'color': 11, 'sequence': 5},
                    ...
                    {'name': _('Cash Payment'), 'type': 'cash_payment', 'code': _('CAPMT'), 'favorite': False, 'sequence': 11},
                    {'name': _('Bank Payment'), 'type': 'bank_payment', 'code': _('BAPMT'), 'favorite': False, 'sequence': 12},
                    {'name': _('Sales Return'), 'type': 'credit_note', 'code': _('SARET'), 'favorite': False, 'sequence': 13},
                    {'name': _('Purchase Return'), 'type': 'debit_note', 'code': _('PURET'), 'favorite': False, 'sequence': 14},]
        # Aman end
