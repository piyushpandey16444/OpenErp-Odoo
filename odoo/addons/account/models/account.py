...
class AccountAccount(models.Model):
    ...
    #Himanshu 20-10-2020 added the new create function for the code that will be added from backend whenever a new chart of accounts will be made
    @api.model
    def create(self, vals):
        new = super(AccountAccount, self).create(vals)
        if 'code' in vals:
            code_obtained = str(vals.get('code'))[0:3]
            if code_obtained not in ['100', '200']:
                self._cr.execute(
                    """select max(CAST(code AS INT)) from account_account where code like '300%' """)
                code = str(self._cr.fetchall()[0][0])
                if code != "None":
                    x = str(int(code[3:]) + 1)
                    if len(x) >= 1:
                        code = str("300" + x.zfill(3))
                        new['code'] = code
                else:
                    new['code'] = 300001
    ...
    #Himanshu Coa  15-10-2020 changed the field from required to readonly.
    code = fields.Char(size=64, readonly =True, index=True)
    #End Himanshu

    deprecated = fields.Boolean(index=True, default=False)

    #Himanshu Coa 20-09-2020 changed the field name from 'Type' to 'Account Type'
    ...
    #Himanshu Coa 20-09-2020 Added the selection field
    Type = fields.Selection([
        ('Customer', 'Customer'),
        ('Vendor', 'Vendor'),
        ('Employee', 'Employee'),
    ...
    #Himanshu COA  16-09-2020 partner_id is added in the chart of account form such that with the help of it all the fields of vendor and customers can be used.
    partner_id = fields.Many2one('res.partner',help ="To get all the fields associated with vendor and customer")
    #End Himanshu
...
class AccountGroup(models.Model):
    ...
    account_type_id = fields.Many2one('account.account.type', string='Account Type', required=True)

    #Himanshu Coa 31-10-2020 added a new function to change the account_type whenever a parent group is selected
    @api.onchange('parent_id')
    def change_account_type(self):
        val = self.env['account.group'].search([('id','=',self.parent_id.id)]).account_type_id
        self.account_type_id = val
...
class AccountJournal(models.Model):
    ...
    #jatin added 2 options for journals 24-11-2020
    type = fields.Selection([
            ('sale', 'Sale'),
            ('purchase', 'Purchase'),
            ('cash', 'Cash Receipt'),
...
class AccountTax(models.Model):
    ...
    # Aman 18/08/2020 added 3 fields to enter income account and expense account
    income_account_id = fields.Many2one('account.account', company_dependent=True,
                                                 string="Income Account", oldname="property_account_income",
                                                 domain=[('deprecated', '=', False)])
    income_account_export_id = fields.Many2one('account.account', company_dependent=True,
                                                  string="Income Account Export", oldname="property_account_expense",
                                                  domain=[('deprecated', '=', False)])
