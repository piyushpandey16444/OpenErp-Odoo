...
class account_abstract_payment(models.AbstractModel):
    ...
    #jatin made changes on 26-11-2020
...
class account_payment(models.Model):
    ...
    # Aman 10/12/2020 Added company_id check in domain of destination_journal_id field
    @api.onchange('destination_journal_id')
    def check_transfer_to(self):
        return {
            'domain': {
                'destination_journal_id': [('type', 'in', ('bank','bank_payment','cash','cash_payment','contra')), ('company_id', '=', self.env.user.company_id.id)]
            }
        }
    # Aman end
    ...
    # Aman 8/10/2020 Added contra payment type
    # payment_type = fields.Selection(selection_add=[('transfer', 'Internal Transfer'), ('expense', 'Expense Payment')])
    payment_type = fields.Selection(selection_add=[('contra', 'Contra'), ('misc', 'Miscellaneous')])
    ...
    #jatin made changes in type on 26-11-2020
    ...
    move_reconciled = fields.Boolean(compute="_get_move_reconciled", readonly=True)
     # Aman 29/11/2020 Added field for payment_type = misc to select account from account.account
    ledger_account_id = fields.Many2one('account.account', String='Ledger Account')
    # Aman end
    bank_ref_no = fields.Text(help='Bank reference number')

    #salman: code to add for jouranl payment
    journal_payment_count=fields.Integer(compute='journal_payment_count_func')

    def journal_payment_count_func(self):
        if self.move_line_ids:
            count = self.env['account.move'].search([('id','=',self.move_line_ids[0].move_id.id)])
            self.journal_payment_count = len(count)

    def button_journal_payment(self):
        action=self.env.ref('account.action_move_journal_line') 
        result=action.read()[0]
        result['context'] = {}
        ref_id=self.env['account.move'].search([('id','=',self.move_line_ids[0].move_id.id)])
        if len(ref_id)==1:
            res=self.env.ref('account.view.move.form',False)
    ...
    def _compute_journal_domain_and_types(self):
        #jatin changed type on 26-11-2020
        journal_type = ['bank','bank_payment','cash','cash_payment']
        domain = []
        if self.currency_id.is_zero(self.amount) and self.has_invoices:
            ...
            if self.payment_type == 'inbound':
                ...
                journal_type = ['bank_payment','cash_payment']
            elif self.payment_type == 'misc':
                journal_type = ['bank_payment', 'cash_payment', 'bank', 'cash']
            # Aman 8/10/2020 Added domain for contra payment_type
            elif self.payment_type == 'contra':
                journal_type = ['bank', 'cash', 'bank_payment', 'cash_payment', 'contra']
            # Aman end
    ...
    def _onchange_amount(self):
        ...
        # Aman 10/12/2020 Added company_id check in domain of journal_id field
        domain_on_types = [('type', 'in', list(journal_types)), ('company_id', '=', self.env.user.company_id.id)]
    ...
    def _onchange_partner_type(self):
       ...
       if self.partner_type: 
          # Aman 12/11/2020 Added type condition in domain to get only parent
    ...
    def _onchange_payment_type(self):
        ...
        if not self.invoice_ids:
            ...
            if self.payment_type == 'inbound':
                ...
                #jatin added on 26-11-2020
                journal_types={'bank','cash'}
            elif self.payment_type == 'outbound':
                self.partner_type = 'supplier'
                # jatin added on 26-11-2020
                journal_types={'bank_payment','cash_payment'}
            # Aman 29/11/2020 Added condition to pass domain for payment_type = misc
            elif self.payment_type == 'misc':
                journal_types={'bank_payment', 'cash_payment', 'bank', 'cash'}
            # Aman end
            # Aman 8/10/2020 Added domain for contra payment type
            elif self.payment_type == 'contra':
                journal_types={'bank','cash','bank_payment','cash_payment','contra'}
            # Aman end
            else:
                journal_types = {'bank_payment','cash_payment','bank','cash'}
        ...
        if not journal_types:
           journal_types = jrnl_filters['journal_types']
        #journal_types.update(['contra'])
        #journal_types = {'contra',}
        # Aman 10/12/2020 Added company_id check in domain of journal_id field
        res['domain']['journal_id'] = jrnl_filters['domain'] + [('type', 'in', list(journal_types)), ('company_id', '=', self.env.user.company_id.id)]
        return res
    ...
    # shivam code for adding smart button in account.invoice form view
    @api.multi
    def button_journal_entry(self):
        ...
        result['context'] = {}
        pay_id = self.env['account.move'].search([('line_ids.payment_id', '=', self.id)])
        res = self.env.ref('account.view_move_form', False)
        result['views'] = [(res and res.id or False, 'form')]
        result['res_id'] = pay_id.id
    ...
    def post(self):
        ...
        for rec in self:
            ...
            if not rec.name:
                ...
                # Aman 8/10/2020 Generate sequence_code for contra payment type
                if rec.payment_type == 'transfer' or rec.payment_type == 'contra':
                    sequence_code = 'account.payment.transfer'
                elif rec.payment_type == 'misc':
                    sequence_code = 'account.payment.misc'
                else:
            ...
            #Aman 29/12/2020 amount for cash ans bank when payment type is misc
            if rec.payment_type == 'misc' and rec.journal_id.type in ('bank', 'cash'):
                amount = rec.amount
            else:
                amount = rec.amount * (rec.payment_type in ('outbound', 'transfer', 'contra') and 1 or -1)
    ...
    def _get_counterpart_move_line_vals(self, invoice=False):
        if self.payment_type == 'transfer':
            name = self.name
            account_id=self.company_id.transfer_account_id
            #account_id = self.destination_journal_id.default_debit_account_id
        # Aman 8/10/2020 Added condition to add account_id for contra payment type
        elif self.payment_type == 'contra':
            name = self.name
            account_id = self.destination_journal_id.default_debit_account_id
        # Aman end
        # Aman 29/11/2020 Added condition to add account_id for misc
        elif self.payment_type == 'misc':
            name = self.name
        ...
        # Aman 28/10/2020 Added account_id form account.account.table for selected partner
            if self.partner_id:
    ...
    def _get_liquidity_move_line_vals(self, amount):
        ...
        vals = {
            'name': name,
            'account_id': self.payment_type in ('outbound', 'transfer', 'contra') and self.journal_id.default_debit_account_id.id or self.journal_id.default_credit_account_id.id,
            'journal_id': self.journal_id.id,
            'currency_id': self.currency_id != self.company_id.currency_id and self.currency_id.id or False,
        }
        # Aman 29/11/2020 Added condition to update account_id in vals
        if self.payment_type == 'misc':
            vals.update({'account_id': self.ledger_account_id.id})
        # Aman end
