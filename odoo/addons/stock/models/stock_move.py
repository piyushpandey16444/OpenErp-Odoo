...
class StockMove(models.Model):
    ...
    # Piyush: state check fields for freezing move line ids
    @api.multi
    def _compute_state_check(self):
        for rec in self:
            check_for_state = False
            if rec.picking_id.state == "bill_pending":
                check_for_state = True
            else:
                check_for_state = False
            rec.state_check = check_for_state
    ...
    # ravi start at 15/2/2020 for compute fields
    @api.depends('move_order_schedule_lines.rejected_qty')
    def compute_ord_schedule_reject(self):
        flag = False
        # print ("_compute_qty_recd_gate",self)
        for line in self:
            reject_qty = 0
            if line.move_order_schedule_lines and line.picking_type_id.name != 'incoming':
                for sch in line.move_order_schedule_lines:
                    if sch.rejected_qty > 0:
                        reject_qty = reject_qty + sch.rejected_qty
            line.ord_schedule_reject_qty = reject_qty
    ...
    # Salman end
    #avinash-24/07/20
    # from_stock_issues = fields.Boolean(string="From stock issue" ,default=False)
    from_issues = fields.Boolean(string="From stock issue" ,default=False)
    #end avinash

    #avinash-24/07/20 To check if value is from Job work challan form
    # Earlier we dont need this flag becasue in action_done method we can direct do stock out
    # but after adding bill pending state without picking id we can do stock out and from job work form we do not any get picking_id

    from_job_work = fields.Boolean(string="From Job work", default=False)
    #end avinash
    ...
    # Piyush: added one2many field on 30-05-2020
    material_out_line_id = fields.Many2one('material.out.line', string='Material Line')
    as_per_schedule = fields.Boolean(string='As Per Schedule', related='picking_id.as_per_schedule')
    # Piyush: state check fields for freezing move line ids
    state_check = fields.Boolean(compute="_compute_state_check", string="State Check", help="If stock picking state is bill pending move lines will be" \
                                                                                           "read only!")
    # code ends here
    ...
    # Gaurav 2/7/20 commented and removed inverse function from product qty for validation
    ...
    product_qty = fields.Float(
        ...
        help='Quantity in the default UoM of the product')
    # Gaurav end
    # Gaurav 21/4/20 edit for initial demand to so qty in xml and adding schedule qty
    ...
    schedule_qty = fields.Float('Schedule Quantity', )
    # Gaurav end
    ...
    # avinash-27/12/20
    # picking_id2 = fields.Many2one('stock.issues', 'Transfer Reference', index=True, states={'done': [('readonly', True)]})
    ...
    state = fields.Selection([
        ...
        ('assigned', 'Available'),
        # Gaurav 10/6/20 added bill pending state
    ...
    # new fields (Someone for receipt)
    ...
    show_scheduling_detail = fields.Boolean('Show Scheduling Detail',
                                            help="True: If Order scheduling Detail exist for Product")
    order_scheduling_detail_id = fields.Many2one('order.scheduling.detail', 'Scheduling Detail')
    order_scheduling_detail_line_id = fields.Many2one('order.scheduling.detail.line', 'Scheduling Detail')
    move_order_schedule_lines = fields.One2many('stock.move.schedule.line', 'move_id', 'Order Schedule Lines')
    move_sale_order_schedule_lines = fields.One2many('stock.move.schedule.line', 'move_id', 'Order Schedule Lines')
    ord_schedule_qty = fields.Float('Order Schedule Qty', digits=dp.get_precision('Product Unit of Measure'))
    ord_schedule_reject_qty = fields.Float('Order Schedule Reject Qty',
                                           digits=dp.get_precision('Product Unit of Measure'),
                                           compute='compute_ord_schedule_reject', store=True)
    qc_schedule = fields.Boolean('from QC Schedule', default=False)

    # ravi at 16/2/2020
    pending_qty = fields.Float('Pending Qty', digits=dp.get_precision('Product Unit of Measure'))

    picking_id2 = fields.Many2one('stock.issues', 'Transfer Reference')
    ...
    # schedule_so_move_lines = fields.One2many('scheduling.sale.order', 'order_scheduling_id', 'Schedule So lines')
    sale_id = fields.Many2one('sale.order', related='picking_id.sale_id')


    # Piyush: code for validation of available qty   20-03-2020
    # Product Check
    # @api.onchange('quantity_done')
    # def _onchange_quantity_done(self):
    #     if self.product_id and self.product_id.positive_stock:
    #         if self.quantity_done > self.product_id.qty_available:
    #             raise ValidationError('Cannot issue quantity more than available. For {} available quantity '
    #                                   'is {} '.format(self.name, self.product_id.qty_available))
    # action_1 = self.env.ref('stock.action_picking_tree_all_sale').id  #Dispatch

    # Company Check
    # @api.onchange('quantity_done')
    # def _onchange_quantity_done(self):
    #     if self.product_id and self.picking_id.picking_type_code == 'outgoing' and self.env.user.company_id.default_positive_stock:
    #         if self.quantity_done > self.product_id.qty_available:
    #             raise ValidationError('Cannot issue quantity more than available. For {} available quantity '
    #                                   'is {} '.format(self.name, self.product_id.qty_available))

    # salman: code to add picking_type_code==internal
    @api.onchange('quantity_done')
    def _onchange_quantity_done(self):
        if self.product_id and (self.picking_id.picking_type_code == 'outgoing' or self.picking_id.picking_type_code == 'internal') and \
                self.env.user.company_id.default_positive_stock and self.product_id.tracking == 'none':
            if self.quantity_done > self.product_uom_qty:
                raise ValidationError('Cannot issue quantity more than reserved. For {} reserved quantity '
                                      'is {} '.format(self.product_id.name, self.product_uom_qty))
    ...
    def _onchange_order_scheduling_id(self):
        ...
        if self.order_scheduling_id and self.order_scheduling_id.purchase_id.order_type == 'open_order':
            if self.order_scheduling_id.order_scheduling_line_ids:
                all_schedule_product = [line.product_id.id for line in
                                        self.order_scheduling_id.order_scheduling_line_ids]
                result['domain'] = {
                    'product_id': [('id', 'in', tuple(all_schedule_product)), ('type', 'in', ['product', 'consu'])]}
        return result
    ...
    def onchange_product_id(self):
        ...
        if self.order_scheduling_id:
            scheduling_line = self.env['order.scheduling.line'].search(
                [('product_id', '=', self.product_id.id), ('order_scheduling_id', '=', self.order_scheduling_id.id),
                 ('company_id', '=', self.env.user.company_id.id)])
            if scheduling_line:
                self.order_scheduling_line_id = scheduling_line[0].id
                scheduling_detail = self.env['order.scheduling.detail'].search(
                    [('order_scheduling_line_id', '=', scheduling_line[0].id),
                     ('company_id', '=', self.env.user.company_id.id)])
                if scheduling_detail:
                    self.order_scheduling_detail_id = scheduling_detail[0].id
                    if self.picking_type_id.code == 'incoming' and self.picking_type_id.warehouse_id and self.picking_type_id.warehouse_id.reception_steps == 'one_step':
                        self.show_scheduling_detail = True
                    elif self.picking_type_id.code == 'internal' and self.picking_type_id.warehouse_id and self.picking_type_id.warehouse_id.reception_steps == 'two_steps':
                        self.show_scheduling_detail = True
                    elif self.picking_type_id.code == 'internal' and self.picking_type_id.warehouse_id and self.picking_type_id.warehouse_id.reception_steps == 'three_steps' and self.location_dest_id.id != self.picking_type_id.warehouse_id.wh_qc_stock_loc_id.id:
                        self.show_scheduling_detail = True
            else:
                self.order_scheduling_line_id = False
                self.order_scheduling_detail_id = False
                self.show_scheduling_detail = False
        return result
    ...
    def _compute_show_details_visible(self):
        ...
        for move in self:
            ...
            # Piyush: code for button to be visible in dispatch on 07-05-2020
            is_dispatch = False
            if move.picking_code == 'outgoing' and self.user_has_groups('stock.group_tracking_lot'):
                is_dispatch = True
            elif move.picking_code == 'outgoing' and move.product_id.tracking != 'none':
                is_dispatch = True
            ...
            #         and (multi_locations_enabled or move.has_tracking != 'none' or len(
            #     move.move_line_ids) > 1 or consignment_enabled or has_package):
            if move.picking_id.picking_type_id.show_operations is False \
                    and (move.state != 'draft' or (not self._context.get('planned_picking') and move.state == 'draft')) \
                    and (multi_locations_enabled or move.has_tracking != 'none' or len(
                move.move_line_ids) > 1 or has_package or is_dispatch or consignment_enabled):
    ...
    def _quantity_done_compute(self):
        ...
        for move in self:
            quantity_done = 0
            if move.move_sale_order_schedule_lines and move.picking_id.picking_type_code == 'outgoing':
                for qty_dispatch in move.move_sale_order_schedule_lines:
                    quantity_done += qty_dispatch.product_uom._compute_quantity(qty_dispatch.dispatch_qty,
                                                                                move.product_uom, round=False)
        ...
        #         quantity_done += move_line.product_uom_id._compute_quantity(move_line.qty_done, move.product_uom, round=False)
        #     move.quantity_done = quantity_done
        # Piyush: code for computing qty done from scheule lines in dispatch on 02-07-2020
        # for item in self:
        #     quantity_done = 0
        #     if item.move_sale_order_schedule_lines and item.picking_id.picking_type_code == 'outgoing':
        #         for qty_dispatch in item.move_sale_order_schedule_lines:
        #             quantity_done += qty_dispatch.dispatch_qty
        #     item.quantity_done = quantity_done
        # code ends here

    def _quantity_done_set(self):
        # if self.from_job_work == False:
        ...
        for move in self:
            ...
            if not move_lines:
                ...
                print("Quantitty donnnnnnnnnnnnnnnnnneeeeeeeeee == ",quantity_done,move)
    ...
    def create(self, vals):
        #avinash-27/07/20 Checking if the picking id is from stock issue
        # print(vals)
        if vals.get('from_issues'):
            # TDE CLEANME: why doing this tracking on picking here ? seems weird
            perform_tracking = not self.env.context.get('mail_notrack') and vals.get('picking_id2')

            if perform_tracking:
                picking = self.env['stock.issues'].browse(vals['picking_id2'])
            ...
            if perform_tracking:
                ...
                initial_values = {picking.id: {'state': picking.state}}
                # print(picking.picking_type_id,picking.picking_type_id.name, picking.name)
        ...
        # ravi start at 15/2/2020
        if 'product_uom_qty' in vals and vals['product_uom_qty'] > 0:
            res.update({'pending_qty': vals['product_uom_qty']})
        # ravi end
        return res
    ...
    def write(self, vals):
        ...
        # Aman 07/08/2020 Added this function to update pending_qty in dispatch
        if 'product_uom_qty' in vals and vals['product_uom_qty'] > 0:
            self.update({'pending_qty': vals['product_uom_qty']})
        # Aman end
        # ravi start at 16/2/2020
        if 'move_order_schedule_lines' in vals and vals['move_order_schedule_lines']:
            qty = 0
            for sch_lines_check in self.move_order_schedule_lines:
                if sch_lines_check.received_qty > 0 :
                    qty = qty + sch_lines_check.received_qty
                self.update({'quantity_done': qty})
        return res
    ...
    def _get_new_picking_values(self):
        ...
        return {
            'origin': self.origin,
            # Aman 06/11/2020 Added delivery_date to dispatch from sale order
            'delivery_date': self.sale_line_id.order_id.manufacturnig_end_date,
    ...
    def _action_confirm(self, merge=True, merge_into=False):
        ...
        for move in self:
            ...
            if move.move_orig_ids:
                ...
                # Gaurav 21/8/20 added to bypass MTO reserve functionality in case of mrp uninstalled
    ...
    def _action_assign(self):
        ...
        # Gaurav 12/6/20 added and commented for adding bill pending state
        # for move in self.filtered(lambda m: m.state in ['confirmed', 'waiting', 'partially_available']):
        for move in self.filtered(lambda m: m.state in ['confirmed', 'waiting', 'partially_available']):
            # #salman add hsn_id in stock move
            move.hsn_id=move.product_id.hsn_id.hsn_code
            #salman end
            ...
            if move.location_id.should_bypass_reservation()\
                ...
                if not move.move_orig_ids:
                    # Gaurav 28/5/20 added to bypass MTO reserve functionality in case of mrp uninstalled
                    mfg_installed = self.env['ir.module.module'].search([('name', '=', 'mrp')])
    ...
    def _create_extra_move(self):
        ...
        # avinash: 20/10/20 Commented and added becasue no need to create extra move when move is from issue form.
        ...
        if not self.picking_id2 and float_compare(self.quantity_done, self.product_uom_qty, precision_rounding=rounding) > 0:
        # end avinash
    ...
    def _action_done(self):
        ...
        for move in moves:
            if move.state == 'cancel' or move.quantity_done <= 0:
                # print("extra moves continue")
                continue
            # extra move will not be merged in mrp

            # avinash -28/07/20 comment to check if picking id2 from issue is there or not for sequence in form
            # if not move.picking_id:
            #     moves_todo |= move


            #avinash - 28/07/20 Check if the picking id is from stock issue or not
            if move.picking_id2:
                pass
            ...
            # print("moves_todo xxxx", moves_todo)
            # end avinash
        ...
        # print(moves_todo)
        # print(moves_todo[0].picking_id)
        # print(moves_todo[0].picking_id2)

        #avinash-28/07/20 Picking id2 is from (stock.issue) form here checking that picking id is from issue form or not
        # If from issue form assign to picking.
        # If not from issue normal flow & picking id is from (stock.picking)
        if moves_todo and moves_todo[0].picking_id2:
            picking = moves_todo and moves_todo[0].picking_id2 or False

        # Earlier there was one if condition modify it into if else
        else:
            picking = moves_todo and moves_todo[0].picking_id or False

        #end avinash

        # avinash-28/07/20 Comment below code to get picking id of stock.issue and stock.picking
        # picking = moves_todo and moves_todo[0].picking_id or False
        #end avinash
        # print("picking," , picking, picking.picking_type_id.name, picking.bill_available, picking.state)

        #---------------------------------------------------
        # Gaurav 12/6/20 added for: if receipt stock picking then check for bill available or not validation
        # Gaurav : if bill not avlble ->bill pending, if avilable ->Done with invoiced date
        # if self.env['stock.picking.type'].browse(self.picking_id.picking_type_id.id).name == 'Receipts':
        # if picking:
        if picking and picking.picking_type_id.name == 'Receipts':
            #Jatin added code to change state to done in case of job work challan on 14-08-2020
            job_order = picking.purchase_id.job_order
            print("job_order", job_order)
            if picking.bill_available == False and picking.state == 'bill_pending':
                # print("if process backorder raise")
                raise ValidationError("You cannot validate this receipt, Bill should be Available !")
            elif picking.bill_available == False  and job_order == False:
                # print("elif process backorder")
                # return self.write({'state' : 'bill_pending'})
                moves_todo.update({'state': 'bill_pending', 'date': fields.Datetime.now()})
                # moves_todo.mapped('move_dest_ids')._action_assign()
            else:
                # print("action done else", moves_todo, picking.delivery_invoice_date)
                # state done when bill avlble with date invoice date
                if job_order == True:
                    moves_todo.write({'state': 'done', 'date': fields.Datetime.now()})
                else:
                    moves_todo.write({'state': 'done', 'date': picking.delivery_invoice_date})
                moves_todo.mapped('move_dest_ids')._action_assign()
            #jatin


        # avinash - 01/08/20 To check if stock move from job work challan
        # elif not picking and self.from_job_work:
        #     moves_todo.write({'state': 'done', 'date': fields.Datetime.now()})
        #     moves_todo.mapped('move_dest_ids')._action_assign()
        ...
        # end avinash
            ...
            # print("action done else last..")
            moves_todo.write({'state': 'done', 'date': fields.Datetime.now()})
            moves_todo.mapped('move_dest_ids')._action_assign()

        # Himanshu product 20-09-2020 if the state of the product is done the write the product_transaction to true
        #This is done for freezing the tracking

        # if moves_todo or moves_todo.product_id: self.env['product.template'].search(
        #     [('company_id', '=', self.env.user.company_id.id),
        #      ('id', '=', moves_todo.product_id.product_tmpl_id.id)]).write({'product_transaction': True})
        for move in moves_todo:
            if move or move.product_id: self.env['product.template'].search(
                        [('company_id', '=', self.env.user.company_id.id),
                         ('id', '=', move.product_id.product_tmpl_id.id)]).write({'product_transaction': True})
        ...
        #avinash - 28/07/20 Issue form have no backorder
        if picking and picking.picking_type_id.name == 'Issue':
            pass

        elif picking:
            # print("picking backorder")
            picking._create_backorder()

        # end avinash


#avinash-28/07/20 comment to check if picking is from issue above if from issue then no back order
        # if picking:
        #     print("picking backorder")
        #     picking._create_backorder()
#end comment
    ...
    def order_scheduling_detail(self):
        """"
        Return "Order Scheduling Detail" Form on Scheduling Line "Detail" button
        """
        scheduling_detail_id = False
        self.ensure_one()

        if self.picking_id.picking_type_code == 'incoming':
            if len(self.move_order_schedule_lines) > 0:
                view = self.env.ref('stock.stock_move_order_schedule_form')
                return {
                    'name': _('Order Schedule Operations'),
                    'type': 'ir.actions.act_window',
                    'view_type': 'form',
                    'view_mode': 'form',
                    'res_model': 'stock.move',
                ...
                sch_list = []
                if self.order_scheduling_line_id:
                    for line in self.order_scheduling_line_id.order_scheduling_detail_line_ids:
                        # print ("linessssssssssss", line, self,line.state)
                        if line.state != 'done':
                            pending_qty = 0
                            if line.rejected_qty > 0:
                                pending_qty = line.product_qty - line.received_qty + line.rejected_qty
                            else:
                                pending_qty = line.product_qty - line.received_qty
                            schedult_tup = (0, False, {
                                'order_scheduling_id': self.order_scheduling_id.id,
                                'order_scheduling_line_id': self.order_scheduling_line_id.id,
                                'order_scheduling_detail_line_id': line.id,
                                ...
                                'date': line.date,
                                'picking_id': self.picking_id.id,
                                # 'picking_date': self.picking_id.date_receipt

                            })
                            sch_list.append(schedult_tup)
                # print ("sch listttttttttttt", sch_list)
                self.move_order_schedule_lines = sch_list
                view = self.env.ref('stock.stock_move_order_schedule_form')
                return {
                    'name': _('Order Schedule Operations'),
                    'type': 'ir.actions.act_window',
        ...
        #avinash:04/09/20 Commented this code because
        #1. On dispatch form (type scheduling) no need of scheduling lines whose pending quantity is zero.
        #2. Also the second else condition of this else part is not important according to my changes.
           # Only wizard open no need to add any lines.

        # Piyush: code for getting else part i.e dispatch part function on click of scheduling button on 27-06-2020

        # else:
        #     self.ensure_one()
        #     if len(self.move_order_schedule_lines) > 0:
        #         so_schedule_id = self.env['scheduling.sale.order'].search(
        #             [('schedule_order_id', '=', self.sale_line_id.id)])
        ...
        #                 'sale_scheduling_id': self.sale_scheduling_id.id,
        #                 'sale_scheduling_line_id': self.sale_scheduling_line_id.id,
        #                 'sale_scheduling_detail_line_id': line.id,
        #                 'product_id': line.product_id.id or False,
        #                 'product_uom': line.product_uom.id or line.product_id.uom_po_id.id,
        #                 'product_qty': line.qty_to_dispatch or 0.0,
        #                 'pending_qty': line.pending_qty,
        #                 'date': line.schedule_date or fields.datetime.now(),
        #                 'picking_id': self.picking_id.id,
        #             }
        #             sch_pick_id = self.env['scheduling.sale.order'].create(schedult_tup)
        #             if len(sch_pick_id) > 0:
        #                 sch_pick_id.write({'move_id': self.id})
        #             elif line.state == 'draft':
        #                 line.update({'move_id': self.id})
        #             view = self.env.ref('stock.view_stock_move_so_dispatch_schedule')
        #             return {
        ...
        #     else:
        #         sch_list = []
        #         # if self.sale_scheduling_line_id:
        #         # print("yupppppppp")
        #         for item in self:
        #             so_schedule_id = self.env['scheduling.sale.order'].search(
        #                 [('schedule_order_id', '=', self.sale_line_id.id)])
        ...
        #                 for line in so_schedule_id:
        #                     # for line in dline.schedules_product_lines:
        #                     #     if line.state != 'done':
        #                     schedult_tup = (0, False, {
        #                         'sale_scheduling_id': self.sale_scheduling_id.id,
        #                         'sale_scheduling_line_id': self.sale_scheduling_line_id.id,
        ...
        #                     })
        #                     sch_list.append(schedult_tup)
        #             self.move_sale_order_schedule_lines = sch_list
        #             view = self.env.ref('stock.view_stock_move_so_dispatch_schedule')
        #             return {
        #                 'name': _('Sale Order Schedule Operations'),
        #                 'type': 'ir.actions.act_window',
        #                 'view_type': 'form',
        #                 'view_mode': 'form',
        #                 'res_model': 'stock.move',
        ...
        #             }
            ...
            self.ensure_one()
            if len(self.move_order_schedule_lines) > 0:
                so_schedule_id = self.env['scheduling.sale.order'].search([('schedule_order_id', '=', self.sale_line_id.id)])
                for line in so_schedule_id:
                    if line.pending_qty > 0:
                        schedult_tup = {
                            ...
                            'date': line.schedule_date or fields.datetime.now(),
                            'picking_id': self.picking_id.id,
                            'schedule_document': line.order_scheduling_id.name or '',
                        }
    ...
    # Gaurav 22/4/20 added button scheduling function(dispatch/scheduling)
    # def sale_order_scheduling_detail(self):
    #     """"
    #     Return "Sales Order Scheduling Detail" Form on Scheduling Line "Detail" button (Dispatch)
    #     """
    #     scheduling_detail_id = False
    #     self.ensure_one()
    #
    #     if self.picking_id.picking_type_code == 'outgoing':
            # if len(self.move_order_schedule_lines) > 0:
            #     view = self.env.ref('stock.view_stock_move_so_schedule')
            #     return {
            #         'name': _('Sale Order Schedule Operations'),
            #         'type': 'ir.actions.act_window',
            #         'view_type': 'form',
            ...
            #     sch_list = []
            #     if self.sale_scheduling_line_id:
            #         for line in self.sale_scheduling_line_id.schedules_product_lines:
            #             # print ("linessssssssssss", line, self,line.state)
            #             if line.state != 'done':
            #                 pending_qty = 0
            #                 # if line.rejected_qty > 0:
...
class StockMoveScheduleLine(models.Model):
    _description = 'Stock Move Scheduling Line'
    _name = "stock.move.schedule.line"
    _inherit = ['mail.thread']

    order_scheduling_id = fields.Many2one('order.scheduling', 'Order Scheduling')
    order_scheduling_line_id = fields.Many2one('order.scheduling.line', 'Order Scheduling Line')
    order_scheduling_detail_line_id = fields.Many2one('order.scheduling.detail.line', 'Order Scheduling Detail Line')
    move_id = fields.Many2one('stock.move', 'Move')
    product_id = fields.Many2one('product.product', 'Product')
    product_uom = fields.Many2one('product.uom', 'Unit of Measure')
    product_qty = fields.Float(string="Schedule Qty", required=True)
    schedule_document = fields.Char(string="Schedule Document")
    ...
    move_state = fields.Selection([('draft', 'Draft'),
                              ('done', 'Done'),
                              ('cancel', 'Cancelled')],
                             'Status', track_visibility='onchange',
                             copy=False, default='draft')
    edit_reject = fields.Boolean('Edit Reject', default=False)
    short_supply_done = fields.Boolean('Short Supply',default=False)
    qc_pass_qty = fields.Float('QC Pass Qty')
    # avinash:12/11/20 Added so that when form is in bill pending state this is true.
    check_bill_pending = fields.Boolean('Check Bill pending', store=True)
    # end avinash

    # Piyush: code for comparing pending and scheduled qty 23-04-2020
    # @api.multi
    # @api.constrains('received_qty')
    @api.onchange('received_qty')
    def _check_received_qty(self):
        """
        Check received_qty should not be more then schedule and pending qty
        """
        # for line in self:
        #     if line.received_qty > 0:
        #         if line.received_qty > line.pending_qty:
        #             raise Warning(_('Cannot Receive Qty more then Pending Qty !'))

        for line in self:
            if line.received_qty > 0 and line.picking_type_code == 'incoming':
                if line.received_qty > line.pending_qty:
                    message = _('Your receiving quantity is more than the pending quantity. !')
                    mess = {
                        'title': _('Check your quantity!'),
                        'message': message
                    }
                    return {'warning': mess}
    ...
    # Piyush: code for getting pending qty from dispatch_qty on 02-07-2020
    # @api.onchange('dispatch_qty')
    # def onchange_dispatch_qty(self):
    #     for data in self:
    #         pending_qty = 0.0
    #         if data.dispatch_qty and data.dispatch_qty < data.product_qty:
    #             pending_qty = data.product_qty - data.dispatch_qty
    #         elif data.dispatch_qty and data.dispatch_qty > data.product_qty:
    #             raise ValidationError('Done Quantity can not be more than Scheduled Quantity!')
    #         data.pending_qty = pending_qty
    # code ends here

    @api.onchange('qc_pass_qty')
    def onchange_qc_pass_qty(self):
        reject_qty = 0.0
        if self.qc_pass_qty:
            if self.qc_pass_qty > self.received_qty:
                raise ValidationError(_('Reject Qty should be less then Received Qty!'))
            else:
                reject_qty = self.received_qty - self.qc_pass_qty
        self.rejected_qty = reject_qty

    # Piyush: code for create on 17-07-2020
    def create(self, vals):
        res = super(StockMoveScheduleLine, self).create(vals)
        return res

    def write(self, records):
        # Piyush: code for validation in scheduling move quantities on 17-07-2020
        if 'dispatch_qty' in records:  # checking if there is dispatch qty in scheduling lines and fetching it
            dispatch_qty = records.get('dispatch_qty')
            if dispatch_qty > self.pending_qty:  # if dispatch is more than pending then error
                raise UserError(_("Done quantity can not be greater than Pending Quantity!"))

            # avinash:02/09/20 Commented because this functionality works on confirm button of dispatch
            # else:
            #     records['pending_qty'] = self.product_qty - dispatch_qty  # pending is schedule - done
            # end avinash
        # code ends here
        result = super(StockMoveScheduleLine, self).write(records)
        return result
    # code ends here
