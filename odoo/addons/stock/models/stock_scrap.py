...
class StockScrap(models.Model):
    ...
    scrap_qty = fields.Float('Quantity', required=True, states={'done': [('readonly', True)]})
    state = fields.Selection([
        ('draft', 'Draft'),
        ('done', 'Done')], string='Status', default="draft")
    date_expected = fields.Datetime('Expected Date', default=fields.Datetime.now)

    # Piyush: validation for inhand quantity and scrap qty  20-03-2020

    # Product check
    # @api.onchange('scrap_qty')
    # def _onchange_available_qty(self):
    #     if self.scrap_qty and self.product_id and self.product_id.positive_stock:
    #         if self.product_id.positive_stock:
    #             if self.scrap_qty > self.product_id.qty_available:
    ...
    @api.onchange('scrap_qty')
    def _onchange_available_qty(self):
        if self.env.context.get('onchange_check'):
            if self.env.user.company_id.default_positive_stock and self.scrap_qty and self.product_id:
                if self.scrap_qty > self.product_id.qty_available:
                    raise ValidationError('Cannot issue quantity more than available. For {} available quantity '
                                          'is {} '.format(self.product_id.name, self.product_id.qty_available))
    # code ends here
...
#             'restrict_partner_id': self.owner_id.id,
    ...
    def _available_quantity(self):
        if self.env.context.get('onchange_check'):
            if self.env.user.company_id.default_positive_stock and self.product_id and self.scrap_qty:
                if self.scrap_qty > self.product_id.qty_available:
                    raise ValidationError('Cannot issue quantity more than available. For {} available quantity '
                                          'is {} '.format(self.product_id.name, self.product_id.qty_available))
        # code ends here
