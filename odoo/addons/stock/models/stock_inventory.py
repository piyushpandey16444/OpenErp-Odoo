...
class Inventory(models.Model):
    ...
    # Aman 24/10/2020 Added readonly condition for cancel state
    ...
    def _selection_filter(self):
        ...
        #Himanshu: 30-07-2020 removed the pack selection option for inventory_adjustment
        # if self.user_has_groups('stock.group_tracking_lot'):h
    ...
    def action_done(self):
        #Himanshu: 30-07-2020 code added to raise the error if the line_id is empty in the inventory adjustment
        for rec in self.line_ids:
            if rec.id== False:
                raise ValidationError(_("There must be an product added in the product line to validate"))
        #End Himanshu
    ...
    def action_cancel_draft(self):
        ...
        # Aman 24/10/2020 Set state to cancel on pressing cancel inventory button and commented previous code
        self.state = 'cancel'
        # self.write({
        #     'line_ids': [(5,)],
        #     'state': 'draft'
    ...
    def action_start(self):
        for inventory in self.filtered(lambda x: x.state not in ('done','cancel')):
            # Aman 12/09/2020 Added this code to add pending quantity in stock.inventory.line
            if inventory.sale_id.order_line:
                mto = inventory.sale_id.order_line
                mto_exist = mto.filtered(lambda mto: mto.product_id.route_ids.name == 'Make To Order' and mto.pending_quantity)
                for k in mto_exist:
                    for j in inventory.line_ids:
                        if k.product_id == j.product_id:
                            res = {'pending_qty': k.pending_quantity}
                            self.env['stock.inventory.line'].search([('id', '=', j.id)]).write(res)
                # for (j, k) in zip(inventory.line_ids, mto_exist):
                #     res={'pending_qty':k.pending_quantity}
                #     self.env['stock.inventory.line'].search([('id', '=',j.id)]).write(res)

            #Himanshu 06-10-2020 Added code for freezing tracking when the tracebility is no-tracking and required when tracebility is unique and lots type.

                for rec in mto:
                    if rec.product_id.tracking == 'none':
                        res = {'tracking_check': True}
                        self.env['stock.inventory.line'].search([('product_id', '=', rec.product_id.id)]).write(res)

                    else:
                        res = {'tracking_check': False}
                        self.env['stock.inventory.line'].search([('product_id','=',rec.product_id.id)]).write(res)
    ...
    def _get_inventory_lines_values(self):
        ...
        for product_data in self.env.cr.dictfetchall():
            ...
            #Himanshu 22-09-2020 freezing the lot/serial number field in tree view of inventory adjustment when tracebility is of no-tracking and required the same field when tracebility of lot /serial type.
            product_id = self.env['product.product'].search([('id','=',product_data['product_id'])]).product_tmpl_id
            # print("product_id........",product_id)
            tracking = self.env['product.template'].search([('id','=',product_id.id)]).tracking
            print("product_id",product_id)
            if tracking == 'none':
                product_data['tracking_check'] = True

            else:
                product_data['tracking_check'] = False
            #End Himanshu
...
class InventoryLine(models.Model):
    ...
    #Shivam 20-10-2020 changed the field name to existing quantity instead of theoritical_qty
    theoretical_qty = fields.Float(
        'Existing Quantity', compute='_compute_theoretical_qty',
    ...
    # Himanshu:05-08-2020 Added this code to make lot/serial number field required/readonly when tracking is true/false
    tracking_check = fields.Boolean(default=False)
    @api.onchange('product_id')
    def change_tracking(self):
        if self.product_id.product_tmpl_id.tracking == 'none':
            self.tracking_check = True
        else:
            self.tracking_check = False
    #end Himanshu
