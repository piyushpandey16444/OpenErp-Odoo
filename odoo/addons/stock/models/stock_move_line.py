...
class StockMoveLine(models.Model):
    ...
    #avinash:13/08/20 Add so that we can compare done quantity with the available quantity for a lot

    available_qty_lot = fields.Float('Available Qty', readonly=True, default=0.0)

    #end avinash

    # avinash:16/10/20 Add so that we can check product tracking type
    check_lot_available = fields.Boolean(readonly=True, store=True)
    # end avinash
    ...
    # Piyush: code for as_per_schedule on 17-07-2020
    as_per_schedule = fields.Boolean(string='As Per Schedule', related='picking_id.as_per_schedule',
                                     help="Used for making done qty readonly for case when lot product with scheduling"
                                          " is available!")
    # Piyush: state check fields for freezing move line ids
    state_check = fields.Boolean(compute="_compute_state_check", string="State Check",
                                 help="If stock picking state is bill pending move lines will be" \
                                      "read only!")
    # code ends here
    ...
    product_qty = fields.Float(
        'Real Reserved Quantity', digits=0,
        compute='_compute_product_qty', store=True)
    # Gaurav end
    product_uom_qty = fields.Float('Reserved', default=0.0, digits=dp.get_precision('Product Unit of Measure'),
                                   required=True)
    ordered_qty = fields.Float('Ordered Quantity', digits=dp.get_precision('Product Unit of Measure'))
    qty_done = fields.Float('Done', default=0.0, digits=dp.get_precision('Product Unit of Measure'), copy=False)
    package_id = fields.Many2one('stock.quant.package', 'Individual Box Label', ondelete='restrict')
    ...
    def _onchange_quantity_done(self):
        # avinash 31/07/20 Check positive stock for No tracking, lot and unique product
        if self.qty_done and self.picking_id2.picking_type_id.name == 'Issue' and self.product_id.type == 'product' \
                and self.product_id.tracking == 'none' and self.env.user.company_id.default_positive_stock:
            if self.qty_done > self.product_id.qty_available:
                raise ValidationError(
                    'Cannot issue quantity more than available. For {} available quantity '
                    'is {} '.format(self.product_id.name, self.product_id.qty_available))

        if self.qty_done and self.picking_id2.picking_type_id.name == 'Issue' and self.product_id.type == 'product' \
                and self.product_id.tracking not in ['none'] and self.env.user.company_id.default_positive_stock:
            lot = self.env['stock.quant'].search(
                [('lot_id', '=', self.lot_id.id), ('product_id', '=', self.product_id.id),
                 ('location_id', '=', self.location_id.id)])
            print(lot.quantity)
            if self.qty_done > lot.quantity:
                raise ValidationError(
                    'Cannot issue quantity more than available. For {} the available quantity in the lot {} '
                    'is {} '.format(self.product_id.name, self.lot_id.name, lot.quantity))

            # end avinash

        if self.product_id and self.picking_id.picking_type_code == 'outgoing' and \
        ...
        # avinash: 13/08/20 Positive check of company for lot wise item for dispatch
        if self.product_id and self.picking_id.picking_type_code == 'outgoing' and \
                self.env.user.company_id.default_positive_stock and self.product_id.tracking not in ['none']:
            if self.product_uom_qty:
                if self.qty_done > self.product_uom_qty:
                    raise ValidationError(
                        'Cannot issue quantity more than available. For {} the available quantity in the lot {} '
                        'is {} '.format(self.product_id.name, self.lot_id.name, self.product_uom_qty))
            elif self.available_qty_lot and not self.product_uom_qty:
                if self.qty_done > self.available_qty_lot:
                    raise ValidationError(
                        'Cannot issue quantity more than available. For {} the available quantity in the lot {} '
                        'is {} '.format(self.product_id.name, self.lot_id.name, self.available_qty_lot))

        #End avinash



    # avinash: 14/07/20  Earlier working (dispatch form): Done quantity compared with reserved quantity and if someone add new lot then reserved
    # quantity was not showing because functionality of reserved quantity works on check avaity button.
    # (My change) : Add available quantity for lot wise item.
    # so when user select a new lot then we can compare done quantity with the available quantity.

    @api.onchange('lot_id')
    def _onchange_lot_id(self):
        if self.lot_id and not self.product_uom_qty and self.picking_id.picking_type_id.name == "Delivery Orders":
            for val in self:
                dest_location_req = ''
                ...
                if location_ids:
                    dest_location_req = location_ids[0].id
    ...
    # Piyush: code for domain in individual box 30-04-2020

    @api.model
    def get_individual_prod_domain(self):
        pack_list = []
        pack_mat = []
        all_pm = self.env['product.product'].search([('categ_id.name', '=', 'Packing Material')])

        if all_pm:
            pack_mat = all_pm.ids

        if 'active_model' in self._context and 'active_id' in self._context:
            if self.env[self._context['active_model']] == self.env['stock.move']:
                move_id = self.env[self._context['active_model']].browse(self._context['active_id'])

                packing_ids = self.env['product.packaging'].search(
                    [('product_id', '=', move_id.product_id.id)])

                if packing_ids:
                    pack_list = []
                    for val in packing_ids:
                        pack_list.append(val.individual_box_item_id.id)

        if pack_list:
            domain = [('id', 'in', pack_list)]
        else:
            domain = [('id', 'in', pack_mat)]

        return domain

    individual_box = fields.Many2one('product.product', 'Individual Box',
                                     domain=get_individual_prod_domain)

    individual_box_item_qty = fields.Float(string='Box Qty')
    readonly_check = fields.Boolean('autofill', help="field added to check the package attached to the individual box "
                                                     "or not and on the basis of check make qty readonly")
    qty_req_true = fields.Boolean('autofill', help="field added to check the package attached to the individual box "
                                                   "or not and on the basis of check make qty required")

    master_box_item_id = fields.Many2one('product.product', 'Master Box',
                                         help="Select the product category")

    # Piyush: code for making master box and label mandatory on click on 05-05-2020
    result_package_id_check = fields.Boolean(string='result package id check', default=False)
    ...
    result_package_id = fields.Many2one(
        'stock.quant.package', 'Master Box Label',
        ondelete='restrict',
    ...
    consume_line_ids = fields.Many2many('stock.move.line', 'stock_move_line_consume_rel', 'consume_line_id',
                                        'produce_line_id', help="Technical link to see who consumed what. ")
    produce_line_ids = fields.Many2many('stock.move.line', 'stock_move_line_consume_rel', 'produce_line_id',
                                        'consume_line_id',
    ...
    # Piyush: code for getting qty in package 30-04-2020
    @api.onchange('individual_box', 'qty_done')
    def individual_box_qty_change(self):
        qty_req_true = False
        individual_box_item_qty = 0.0
        if self.individual_box and self.qty_done > 0:

            # Piyush: code for creating qty of individual box item on 01-05-2020
            packaging_ids_values = self.env['product.packaging'].search([('product_id', '=', self.product_id.id),
                                                                         ('individual_box_item_id', '=',
                                                                          self.individual_box.id)])
            if packaging_ids_values:
                self.readonly_check = True
                for val in packaging_ids_values:
                    if packaging_ids_values.qty > 0:
                        individual_box_item_qty = math.ceil(self.qty_done / packaging_ids_values.qty)
                        self.individual_box_item_qty = individual_box_item_qty

            qty_req_true = True
            self.qty_req_true = qty_req_true

    # code for onchange of result_package_id on 05-05-2020
    # @api.multi
    # @api.depends('picking_id.picking_type_id.code')
    # def _picking_type_code_value(self):
    #     for move_line in self:
    #         visible_invisible_dispatch_fields = False
    #         if move_line.picking_id and move_line.picking_id.picking_type_id and move_line.picking_id.picking_type_id.code == 'outgoing':
    #             visible_invisible_dispatch_fields = True
    #         move_line.visible_invisible_dispatch_fields = visible_invisible_dispatch_fields

    @api.onchange('result_package_id', 'master_box_item_id')
    def result_package_id_check_field(self):
        result_package_id_check = False
        if self.result_package_id or self.master_box_item_id:
            result_package_id_check = True
        self.result_package_id_check = result_package_id_check

    # code ends here

    # ravi start at 27/2/2020 for address
    picking_id2 = fields.Many2one(
        'stock.issues', 'Stock Picking',
        help='The stock operation where the packing has been made')
    ...
    def _compute_product_qty(self):
        self.product_qty = self.product_uom_id._compute_quantity(self.product_uom_qty, self.product_id.uom_id,
    ...
    def check_reserved_done_quantity(self):
        for move_line in self:
            if move_line.state == 'done' and not float_is_zero(move_line.product_uom_qty, precision_digits=self.env[
    ...
    def create(self, vals):
        ...
        # Piyush: code for creating values in product packaging 09-05-2020

        if 'individual_box' in vals and vals.get('individual_box'):
            if ml.individual_box_item_qty < 1:
                raise ValidationError("Box Quantity can not be 0, please provide sufficient quantity! ")
            if ml.qty_done > 0 and ml.individual_box_item_qty > 0:
                quantity_given = ml.qty_done % ml.individual_box_item_qty
                if quantity_given == 0:
                    exist_move = self.env['product.packaging'].search([('product_id', '=', ml.product_id.id),
                                                                       ('individual_box_item_id', '=',
                                                                        ml.individual_box.id)])
                    quantity_per_box = ml.qty_done / ml.individual_box_item_qty
                    if not exist_move:
                        self.env['product.packaging'].create({
                            'name': 'Pack of ' + str(int(quantity_per_box)),
                            'product_id': ml.product_id.id,
                            'individual_box_item_id': ml.individual_box.id,
                            'qty': quantity_per_box,
                        })
    ...
    def write(self, vals):
        ...
        # Piyush: code for creating values in product packaging 05-05-2020

        if 'individual_box' in vals and vals.get('individual_box'):
            if self.individual_box_item_qty < 1:
                raise ValidationError("Box Quantity can not be 0, please provide sufficient quantity! ")
            if self.qty_done > 0 and self.individual_box_item_qty > 0:
                quantity_given = self.qty_done % self.individual_box_item_qty
                if quantity_given == 0:
                    exist_move = self.env['product.packaging'].search([('product_id', '=', self.product_id.id),
                                                                       ('individual_box_item_id', '=',
                                                                        self.individual_box.id)])
                    quantity_per_box = self.qty_done / self.individual_box_item_qty
                    if not exist_move:
                        self.env['product.packaging'].create({
                            'name': 'Pack of ' + str(int(quantity_per_box)),
                            'product_id': self.product_id.id,
                            'individual_box_item_id': self.individual_box.id,
                            'qty': quantity_per_box,
                        })
            # else:
            #     new_move = self.env['product.packaging'].search([('product_id', '=', self.product_id.id)])
            #     for pro in new_move:
            #         if pro.individual_box_item_id.name != self.individual_box.name:
            #             self.env['product.packaging'].create({
            #                 'product_id': self.product_id.id,
            #                 'individual_box_item_id': self.individual_box.id,
            #             })
    ...
    def _action_done(self):
        ...
        for ml in self - ml_to_delete:
            if ml.product_id.type == 'product':
                ...
                # ---------------------------
                # Gaurav 30/6/20 added for -> quantity = latest done on stock move(picking)
                # move what's been actually done
                quantity = ml.product_uom_id._compute_quantity(ml.qty_done, ml.move_id.product_id.uom_id, rounding_method='HALF-UP')
                # Gaurav added condition for if receipt picking type and bill not available then condition-> to set quantity = 0
                # so that stock quant will not update or add quantity in case of bill pending
                if ml.move_id.picking_id.picking_type_id.name == 'Receipts' and ml.move_id.picking_id.bill_available == False:
                    quantity = 0
                # Gaurav end------------------
                available_qty, in_date = Quant._update_available_quantity(ml.product_id, ml.location_id, -quantity, lot_id=ml.lot_id, package_id=ml.package_id, owner_id=ml.owner_id)
