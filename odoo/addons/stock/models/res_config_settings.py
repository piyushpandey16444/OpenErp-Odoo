...
class ResConfigSettings(models.TransientModel):
    ...
    default_positive_stock = fields.Boolean("Positive Stock", default_model="product.template")
    # Avinash: 06/11/2020 commented and Added field to install sequence_generator module
    # Aman 17/10/2020 Added field to install stock_ext module
    # module_stock_ext = fields.Boolean("Lot Sequence Generator")
    module_sequence_generator = fields.Boolean("Lot Sequence Generator")

    # Avinash: 06/11/2020 Added onchange function to make module_sequence_generator true when group_stock_production_lot is true and same for false
    # Aman 17/10/2020 Added onchange function to make module_stock_ext true when group_stock_production_lot is true and same for false
    # @api.onchange('group_stock_production_lot')
    # def _onchange_group_stock_production_lot(self):
    #     if self.group_stock_production_lot:
    #         self.module_stock_ext = 1
    #         print(self.module_stock_ext)
    #     else:
    #         self.module_stock_ext = 0
    # Aman end
    @api.onchange('group_stock_production_lot')
    def _onchange_group_stock_production_lot(self):
        if self.group_stock_production_lot:
            self.module_sequence_generator = 1
        else:
            self.module_sequence_generator = 0
    ...
    def set_values(self):
        ...
        # Piyush: code for updating company check 19-03-2020
        user_company = self.env.user.company_id.id
        self.env.cr.execute('''UPDATE res_company SET default_positive_stock = {} WHERE id = {}'''.format(
                self.default_positive_stock, user_company))
        # code ends here
