...
class ProductTemplate(models.Model):
    ...
    # avinash:13/11/20 Added field show_hide_qty Becuase when data in product moves then this button is invisible
    # @api.depends('qty_available')
    @api.multi
    def _show_hide_qty(self):
        move_line = self.env['stock.move.line'].search([('product_id', '=', self.product_variant_id.id)])
        if move_line:
            self.show_hide_qty = True

    show_hide_qty = fields.Boolean(string='Show/Hide Qty Onhand', compute='_show_hide_qty',
                                   help="Qty Onhand button will be hide if value in stock_move_lines")
    ...
    # Piyush changes in the field for migration
    ...
    def write(self, vals):
        ...
        # ravi start at 6/2/2020 for validating single route
        res = super(ProductTemplate, self).write(vals)

        # Piyush: code to check that buy is selected for purchase type order 24-04-2020
        if self.purchase_ok and self.route_ids and self.route_ids.name != 'Buy':
            #jatin changed on 09-07-2020
            raise ValidationError("{} is a purchase type product only Buy route can be selected for it! ".format(self.name))
            #end jatin
        # code ends here

        if self.route_ids:
            print('self.route_ids',self.route_ids)
            print('self.route_ids',len(self.route_ids))
            if len(self.route_ids) > 1:
                raise UserError(_("You can not Select More than One Route "))
        #     Gaurav 19/5/20 bypass route validation in case of s
        elif self.type == 'service':
            pass
        # else:
        #     raise UserError(_("Please Select Route "))
        return res
        # ravi end

    # ravi start at 6/2/2020 for validating single route writing create function
    @api.model
    def create(self, vals):
        ...
        # Piyush: code to check that buy is selected for purchase type order 24-04-2020
        if res.purchase_ok and res.route_ids and res.route_ids.name != 'Buy':
            # jatin changed on 09-07-2020
            raise ValidationError("{} is a purchase type product only Buy route can be selected for it! ".format(self.name))
            #end jatin
        # code ends here
        ...
        if res.route_ids:
            print('self.route_ids',res.route_ids)
            print('self.route_ids',len(res.route_ids))
    ...
    # ravi start at 7/2/2020 for default value of routs
    # @api.onchange('purchase_ok')
    # def onchange_purchase_ok(self):
    #     if self.purchase_ok:
    #         buy_option = self.env['stock.location.route'].search([('name', '=', 'Buy')])
    #         if buy_option:
    #             for val in buy_option:
    #                 self.route_ids = [buy_option.id]
    #     else:
    ...
    def onchange_purchase_ok(self):
        # Gaurav 30/5/20 below functionality is being commented because other functionality"
        # has been added by piyush and this function is being called on other places
        # so we just cant remove this function
        if self.purchase_ok and not self.sale_ok:
            buy_option = self.env['stock.location.route'].search([('name', '=', 'Buy')])
            # self.route_ids = [buy_option.id]
            print("xxxx",buy_option)
        else:
            # self.route_ids = False
            print("xxxx else")
    # ravi end
