...
class ProcurementGroup(models.Model):
    ...
    def run(self, product_id, product_qty, product_uom, location_id, name, origin, values):
        ...
        if not rule:
            # Gaurav 16/5/20 added code with abhishek sir for checking, if mrp is not installed donot raise error
            mfg_search = self.env['ir.module.module'].search([('name','=','mrp')])
            print("xxxx", mfg_search, mfg_search.state)
            if not mfg_search.state == 'installed':
                return True
            else:
    ...
    # Search rule commented and new added for search rule for individual mto by pushkal on 06 feb 19
    # @api.model
        ...
        if not res:
            ...
            if product_routes:
                ...
                if not res:
                    #print("xxxx", res)
                    route_list=[]
                    for val in product_routes:
                        route_list.append(val.name)
                    if len(route_list) > 0:
                        if 'Make To Order' in route_list and 'Manufacture' in route_list:
                            res = Pull.search(expression.AND([[('route_id', 'in', product_routes.ids)], domain]),
                                              order='route_sequence, sequence', limit=1)
                        elif 'Make To Order' in route_list:
                            manf_route = self.env['stock.location.route'].search([('name','=','Manufacture')])
                            if len(manf_route) > 0:
                                res = Pull.search(expression.AND([[('route_id', 'in', manf_route.ids)], domain]),
                                              order='route_sequence, sequence', limit=1)
