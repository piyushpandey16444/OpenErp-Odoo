...
class Warehouse(models.Model):
    ...
    # Gaurav 24/6/20 added function for dynamically turn create on-off on basis of multiple warehouse
    @api.model
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(Warehouse, self).fields_view_get(
    ...
    # Gaurav 7/3/20 modified name of Routes as Process (Product view)
    ...
    route_ids = fields.Many2many(
        'stock.location.route', 'stock_route_warehouse', 'warehouse_id', 'route_id',
        'Process', domain="[('warehouse_selectable', '=', True)]",
        help='Defaults routes through the warehouse')
    # Gaurav end
    ...
    # Gaurav 10/4/20 added trf and iss type for transfer and issue operation type
    trf_type_id = fields.Many2one('stock.picking.type', 'Transfer Type')
    iss_type_id = fields.Many2one('stock.picking.type', 'Issue Type')
    # Gaurav end
    # Aman 27/11/2020 added fields for sales return and purchase return operation type
    sales_return_type_id = fields.Many2one('stock.picking.type', 'Sales Return Type')
    ...
    def create(self, vals):
        ...
        reception_steps_dup = vals.get('reception_steps')
        print("hhh",reception_steps_dup)
    ...
    def create_sequences_and_picking_types(self):
        ...
        # Gaurav 24/6/20 added locations default issue for OT -> Issue
        location_iss_ids = self.env['stock.location'].search(
            [('name', '=', 'Issue')])
        # Gaurav end
        # Aman 27/11/2020 added destination location for purchase return
        dest_location_purchase_id = self.env['stock.location'].search(
            [('name', '=', 'Vendors')])
        # Aman end
        ...
        create_data = {
            ...
            # Gaurav 10/4/20 added trf and iss type for transfer and issue operation type
                ...
                'use_existing_lots': True,
                'default_location_src_id': self.lot_stock_id.id,
                'default_location_dest_id': location_iss_ids.id,
                'sequence': max_sequence + 4,
            },
            #     Gaurav end
            # Aman 27/11/2020 added fields for sales return and purchase return operation type
                'sales_return_type_id': {
                'name': _('Sales Return'),
                'code': 'outgoing',
                'use_create_lots': False,
                'use_existing_lots': True,
                ...
                'use_create_lots': False,
                'use_existing_lots': True,
                'default_location_dest_id': dest_location_purchase_id.id,
                'sequence': max_sequence + 5,
            },
            # Aman end
        }
        ...
        PickingType.browse(warehouse_data['out_type_id']).write({'return_picking_type_id': warehouse_data['sales_return_type_id']})
        PickingType.browse(warehouse_data['in_type_id']).write({'return_picking_type_id': warehouse_data['purchase_return_type_id']})
    ...
    def _update_name_and_code(self, new_name=False, new_code=False):
        ...
        for warehouse in self:
            ...
            # Gaurav 10/4/20 added trf and iss type for transfer and issue operation type
            warehouse.trf_type_id.sequence_id.write(sequence_data['trf_type_id'])
            warehouse.iss_type_id.sequence_id.write(sequence_data['iss_type_id'])
            # Gaurav end
            # Aman 27/11/2020 added fields for sales return and purchase return operation type
            warehouse.sales_return_type_id.sequence_id.write(sequence_data['sales_return_type_id'])
            warehouse.purchase_return_type_id.sequence_id.write(sequence_data['purchase_return_type_id'])
            # Aman end
    ...
    def _get_picking_type_values(self, reception_steps, delivery_steps, pack_stop_location):
        ...
        return {
            ...
            # Gaurav end
            # Aman 27/11/2020 added fields for sales return and purchase return operation type
            'sales_return_type_id': {'default_location_dest_id': output_loc.id},
            'purchase_return_type_id': {'default_location_src_id': input_loc.id},
            # Aman end
        }
    ...
    def _get_sequence_values(self):
        return {
            ...
            # Gaurav 10/4/20 added trf and iss type for transfer and issue operation type
            'trf_type_id': {
                'name': self.name + ' ' + _('Sequence transfer'),
                'prefix': self.code + '/TRF/', 'padding': 5,
                'company_id': self.company_id.id,
            },
            'iss_type_id': {
                'name': self.name + ' ' + _('Sequence issue'),
                'prefix': self.code + '/ISS/', 'padding': 5,
                'company_id': self.company_id.id,
            },
            # Gaurav end
            # Aman 27/11/2020 added fields for sales return and purchase return operation type
            'sales_return_type_id': {
                'name': self.name + ' ' + _('Sequence sales return'),
                'prefix': self.code + '/SALES/', 'padding': 5,
                'company_id': self.company_id.id,
            },
            'purchase_return_type_id': {
                'name': self.name + ' ' + _('Sequence purchase return'),
                'prefix': self.code + '/PURCHASE/', 'padding': 5,
                'company_id': self.company_id.id,
            },
            # Aman end
        }
