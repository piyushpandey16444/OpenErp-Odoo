...
class MailComposer(models.TransientModel):
    ...
    def send_mail(self, auto_commit=False):
        ...
        # Aman 29/09/2020 Set sale quotation state to quotation sent when email is sent
        if self.model == 'sale.quotation':
            quotation = self.env['sale.quotation'].search([('name', '=', self.display_name)])
            if quotation:
                res = {'state': 'sent', 'check_duplicate': False}
                quotation.write(res)
        # Aman end
        for wizard in self:
