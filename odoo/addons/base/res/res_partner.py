...
class Partner(models.Model):
    ...
    city = fields.Char()
    # Gaurav 7/3/20 added required on state (Customer)
    # Piyush: required removed from state (Customer) on 14-10-2020
    ...
    def update_address(self, vals):
        for val in self:
            # Piyush: code for removing singleton error as multiple address are coming of all the childs
            # related to parent on 30-09-2020
            # addr_vals = {key: vals[key] for key in self._address_fields() if key in vals}
            addr_vals = {key: vals[key] for key in val._address_fields() if key in vals}
            if addr_vals:
                # return super(Partner, self).write(addr_vals)
                return super(Partner, val).write(addr_vals)
