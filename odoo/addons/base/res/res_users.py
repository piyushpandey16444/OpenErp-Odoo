...
class Users(models.Model):
    ...
    def create(self, vals):
        ...
        # Gaurav 2/4/20 edit to make every user-->employee (default it was customer)
        print("user.partner_id.active --user------",
              user.partner_id.active,user.partner_id.customer,user.partner_id.supplier,user.partner_id.employee)
        # user.partner_id.customer=False
        # user.partner_id.supplier=False
        # user.partner_id.employee=True
        user.partner_id.write({'customer': False,'supplier': False,'employee': True})
        # Gaurav end
    ...
    def write(self, values):
        ...
        # Gaurav 2/4/20 edit to make every user-->employee (default it was customer)
        print("user.partner_id.active --user------",
              self.partner_id.active, self.partner_id.customer, self.partner_id.supplier, self.partner_id.employee)
        self.partner_id.write({'customer': False, 'supplier': False, 'employee': True})
        # Gaurav end
        ...
        try:
            with cls.pool.cursor() as cr:
                ...
                if user:
                    ...
                    # ravi start at 25/2/2020
                    # self.env.cr.execute(""" select count(*) from res_users where user_login_details = True and company_id = %s """ % (user.company_id.id))
                    # match_recs = self.env.cr.dictfetchall()
                    # if not user.user_login_details and match_recs[0].get('count') < user.company_id.total_number_of_allowed_users:
                    #     self.env.cr.execute(""" update res_users set user_login_details = True where id = %s """ % (user_id))
                    # else:
                    #     user_id = False
                    # ravi end
