...
class Company(models.Model):
    ...
    # Gaurav 7/3/20 added required attrs for state mandate
    ...
    # Aman 3/12/2020 Added pan number field
    pan = fields.Char(string='PAN', help="Permanent Account Number")
    # Aman end
    ...
    # Aman 3/12/2020 Added validation for pan number
    @api.constrains('pan')
    def check_pan_number(self):
        if self.pan and len(self.pan) == 10:
            if re.match(r'^[A-Z, a-z]{5}[0-9]{4}[A-Z, a-z]$', self.pan):
                pan_no = self.pan[0:5].upper()
                pan_no2 = self.pan[9].upper()
                pan_no3 = self.pan[5:9]
                if not self.pan[0:5].isupper() or not self.pan[9].isupper():
                    self.pan = pan_no + pan_no3 + pan_no2
            else:
                raise ValidationError(
                    'Invalid PAN number'
                    '\n Length of PAN number should be 10'
                    '\n First five values should be alphabets'
                    '\n Next four values should be numbers'
                    '\n Last value should be alphabet'
                    '\n e.g. AAFFD3743A'
                )
        else:
            raise ValidationError(
