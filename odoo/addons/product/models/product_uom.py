...
class ProductUoMCategory(models.Model):
    ...
    # Gaurav 29/4/20 added company id field for data transfer to another company
...
class ProductUoM(models.Model):
    ...
    def create(self, values):
        #avinash:12/11/20 Added so that for a category reference unit cannot be added twice
        category_available = self.search([('category_id', '=', values.get('category_id')), ('uom_type', '=', 'reference')])
        if category_available and values.get('uom_type') == 'reference':
            raise ValidationError('Error! You cannot select type reference more than once in same category.')
        #end avinash
