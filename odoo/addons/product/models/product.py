...
class ProductCategory(models.Model):
    ...
    def _check_unique_name(self):
        ...
        for line in self:
            all_temp_list = []
            all_temp = line.env['product.category'].search(
    ...
    # Gaurav 4 march 20 adds company id invisible for unique constrains
    company_id = fields.Many2one(
        'res.company', 'Company',
        default=lambda self: self.env.user.company_id.id, index=1)
    # Gaurav end

    # Piyush: code for making predefined fields readable only 28-04-2020
    predefined_name = fields.Boolean('predefined', compute='predefined_name_check', default=False)

    @api.onchange('name')
    def predefined_name_check(self):
        if self.name == "All":
            self.predefined_name = True
        elif self.name == "Saleable":
            self.predefined_name = True
        elif self.name == "Raw Material":
            self.predefined_name = True
        elif self.name == "Semi Finished":
            self.predefined_name = True
        elif self.name == "Finished":
            self.predefined_name = True
        elif self.name == "Packing Material":
            self.predefined_name = True
        elif self.name == "Original":
            self.predefined_name = True
    # code ends here

    name = fields.Char('Name', readonly="1", index=True, translate=True, required=True)
    ...
    # Piyush: Fields added for migrating data
    ...
    def create(self, vals):
        ...
        #         raise ValidationError(_('The Category is Already Exist'))
...
class ProductProduct(models.Model):
    ...
    # Gaurav 24/4/20 adds saledetail_id for product.saledetail and purchasedetail
    saledetail_id = fields.One2many('product.saledetail', 'product_id', 'Sale product detail')
    purchasedetail_id = fields.One2many('product.purchasedetail', 'product_id', 'Purchase product detail')
...
class ProductPackaging(models.Model):
    ...
    qty = fields.Float('Quantity per Package', default=1.0, help="The total number of products you can have per "
                                                                 "pallet or box.")

    # Piyush: drop down field added in packaging 25-04-2020
    individual_box_item_id = fields.Many2one('product.product', 'Individual Box Item',
                                          help="Select the product category")

    # Piyush: onchange for the check that qty should not be 0 on 07-05-2020
    @api.onchange('qty')
    def qty_check(self):
        if self.qty == 0:
            raise ValidationError('Quantity can not be zero, please increase the number of quantity per box.')
...
# =============Gaurav 24/4/20 edit and added model for sale product detail==============

class SaleProductDetail(models.Model):
    _name = "product.saledetail"
    _description = "Information about a product customer"
    _order = 'sequence desc'

    # Piyush: code for adding fields on 21-06-2020
    ...
    customer_partner_id = fields.Many2one(
        ...
        help="Customer of this product", readonly=True)

    sequence = fields.Char(
        'Reference No.', help="Reference number")

    price = fields.Float(
        'Latest Price', default=0.0, digits=dp.get_precision('Product Price'),
        help="The price to sell a product")
    ...
    product_id = fields.Many2one(
        'product.product', 'Item Variant name')
    ...
    product_detail_tmpl_id = fields.Many2one(
        'product.template', 'Product template',
        index=True, ondelete='cascade')


# =============Gaurav 24/4/20 edit and added model for Purchase product detail==============

class PurchaseProductDetail(models.Model):
    _name = "product.purchasedetail"
    _description = "Information about a product vendor"
    _order = 'sequence desc'
    ...
    date = fields.Date('Date', help="Date for this supplier price")
    ...
    # product_product_pur_id = fields.Many2one(
    #     'product.product', 'Product Product',
    #     index=True, ondelete='cascade')

    product_detail_pur_tmpl_id = fields.Many2one(
        'product.template', 'Product template',
        index=True, ondelete='cascade')
