...
ListController.include({
   renderButtons: function($node) {
        ...
        hide_show_columns : function(){
            ...
            var dataList = [];
            $("#showcb input[type = checkbox]:checked").parent().parent().each(function(){
                dataList.push($(this).text().trim());
            });
            var finaldataList = dataList.join(",");
            if(dataList.length > 0 ){
                // console.log("setCookie123","temp_"+$(".breadcrumb").text().trim().replace(/ /g,"_"), ":", finaldataList);
                setCookie("temp_"+$(".breadcrumb").text().trim().replace(/ /g,"_"),finaldataList,1);
            }
            // End
