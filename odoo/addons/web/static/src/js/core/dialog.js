...
var Dialog = Widget.extend({
    ...
    open: function () {
        ...
        setTimeout(function(){
            var dyn_str = makeid(5);
            // $(".o_input").attr("name_1", dyn_str);
            $(".o_input").removeAttr("name_1");
            $(".o_input").attr("name_1", dyn_str);
        },500)

        // Devashish code start 29 June | for applying autocomplete off on all text box on create mode
        var all_input = document.getElementsByClassName('o_input');
        $('.o_input').attr('autocomplete','off');
        $(all_input).attr('autocomplete','off');
        $('input').attr("autocomplete", "new-password");

        var allnew_input = document.getElementsByClassName('o_field_char');
        $('.o_field_char').attr('autocomplete','off');
        $(allnew_input).attr('autocomplete','off');
        $('input').attr("autocomplete", "new-password");

        var useraddress = document.getElementsByClassName("o_address_street");
        $('.o_address_street').attr('autocomplete','off');
        $(useraddress).find("input").attr('autocomplete','off');
        $('input').attr("autocomplete", "new-password");
        // console.log("useraddress", useraddress);
        // End

        // 03 Dec 2019 Devashish | When we click on listview links, in wizard everything should be on readonly mode,
        // and remove edit and create button
        var modal_body = $(".modal-dialog").length;
        if(modal_body == 1 || modal_body > 1){
            if($(".modal-dialog .modal-title").text().trim() == "Details" || $(".modal-dialog .modal-title").last().text().trim() == "Details"){
                $(".o_form_buttons_edit .o_form_button_save").click();
                $(".modal-footer .save_button").click();
                $(".modal-footer .o_form_buttons_edit").css("display","none");
                $(".modal-body .oe_stat_button").attr("disabled","disabled");
                $(".modal-body .oe_button_box .oe_stat_button").attr("disabled","disabled");
                $(".modal-body .o_statusbar_buttons").css("display","none");
                $(".modal-body .o_field_x2many_list_row_add").css("display","none");
                $(".modal-body .oe_button_box .oe_stat_button:visible").attr("disabled","disabled");
                // $(".modal-body .oe_highlight").attr("disabled","disabled");
                $(".modal-body .o_list_record_delete_btn").attr("disabled","disabled");
                $(".modal-body .o_list_record_delete_btn").css("color","#999999");
                $(".modal-body .o_icon_button").css("color","#999999");
                $(".modal-body .o_icon_button").attr("disabled","disabled");
                $(".modal-body .o_select_file_button").attr("disabled","disabled");
                $(".modal-footer").append("<button class='btn btn-sm btn-default o_form_button_cancel' type='button' data-dismiss='modal'><span>Close</span></button>").one();
                $(".modal-footer .o_form_buttons_view").css("display","none");
                $(".modal-footer button[special=cancel]").css("display","none");
                $(".modal-footer .o_form_button_cancel").nextAll().css("display","none");
                $(".modal-dialog .modal-title").append("<input type='text' id='input1' style='height:2px; width:5px; border:1px solid #ffffff; color:#ffffff;'/>").focusin();
                console.log("Add an item not found ...................................... 03 Dec 2019.");
            }
        }
        else {
            $(".btnnew-special").css("display","none");
        }
        // End
