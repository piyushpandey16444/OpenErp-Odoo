...
var FormController = BasicController.extend({
    ...
    renderSidebar: function ($node) {
        if (!this.sidebar && this.hasSidebar) {
            ...
            // Devashish 20 March 2020 | Code for hide delete button on form view
...
// Devashish code start 01 June | for removing breadcrumbtxt class from readonly inner page mode
$(document).ready(function(){
    
    // Devashish 27 June
    $(window).on('hashchange', function(e){
        tempUrl = location.href;
        // var $datepickerinputfeild = this.$('div').find(".o_field_widget");
        var datepickerinputfeild = document.getElementsByClassName("o_datepicker_input");
        // console.log("datepickerinputfeild",datepickerinputfeild);
        if(tempUrl.indexOf('view_type=form') > -1){
            // $datepickerinputfeild.find(".o_datepicker_input").attr('autocomplete','off');
            $(datepickerinputfeild).attr('autocomplete','off');
            // 17 July devashish code start | remove fixed_header classs from list view when we go to form mode
            var one_manytbl = document.getElementsByClassName("fixed_header");
            // $(one_manytbl).removeClass("fixed_header");
            // End
        }
        else{
            // $datepickerinputfeild.find(".o_datepicker_input").attr('autocomplete','off');
        ...
        // Devashish code start 12 July | Code for column checked on dropdown list stored on cookies.
        setTimeout(function(){
    ...
    // Devashish code start 12 July | Code for column checked on dropdown list stored on cookies.
    // var columnlist = $("#showcb input[type=checkbox]:checked:eq(0)").parent().parent().text().trim();
    // var columnlist = $("#showcb input[type = checkbox]:checked").parent().parent().text().trim();
    ...
    function callGetCookies(){
        var tempVal = getCookie("temp_"+$(".breadcrumb").text().trim().replace(/ /g,"_"));
        // console.log("Getcookies:",tempVal);
        if(tempVal.length > 0){
            var dataList = tempVal.split(',');
            $("#showcb input[type = checkbox]").each(function(index,val){
                var tempText = $(this).parent().parent().text().trim();
                // console.log("loop:",tempText);
                $(this).prop('checked',false);
                if(dataList.indexOf(tempText) > -1){
                    $(this).prop('checked',true);
                }
            });
            $(".oe_dropdown_btn").click();
        }
    }
