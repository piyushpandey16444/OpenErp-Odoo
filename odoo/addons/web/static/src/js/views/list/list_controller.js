...
var ListController = BasicController.extend({
     ...
     * @param {jQuery} $node
     */
    renderButtons: function ($node, a_id) {
        if (!this.noLeaf && this.hasButtons) {
            ...
            // Shivam 02/12/20 code for adding header button on product.template
            this.$buttons.on('click', '.all_product', this._all_product.bind(this));
            this.$buttons.on('click', '.sold_product', this._sold_product.bind(this));
            this.$buttons.on('click', '.purchased_product', this._purchased_product.bind(this));

            // shivam end

            // Devashish 26 Nov | Reload listview when we click on reload button on top
            this.$buttons.on('click', '.oe_reload_listview', this._onReloadListview.bind(this));
            // End

            this.$buttons.appendTo($node);

            //shivam 02/12/20 code for adding header button on product.template
            var small_buttons = this.$buttons.find('.small_buttons');
            $(small_buttons).css("padding-top","3px");
            $(small_buttons).css("padding-bottom","3px");
            $(small_buttons).css("margin-top","5px");


            var all_button = this.$buttons.find('.all_button');
            $(all_button).addClass("active_btn_top");
            // shivam end

            //Shivam Changes the button color

            console.log("############################################ uuuuuuuuuuuuuuuuuuu purchase.product_normal_action_puchased", a_id);

            if(a_id == 'stock.product_template_action_product')
            {
            console.log("############################################ uuuuuuuuuuuuuuuuuuu", a_id);
                var total = this.$buttons.find('.all_product');
                $(total).css('background-color','#fffff');
                $(total).css('color','#4c4c4c');
                $(total).css('font-weight','bold');
                $(total).css('background', btn_color);
//                    ev.stopPropagation();
//                    var self = this;
//
//                    require('web.rpc').query({
//                        model: 'product.template',
//                        method: 'all_product_show',
//                        args: [[]]
//                    }).then(function (result) {
//                        self.do_action(result).done(function(){
//                            $('.all_product').css('background',btn_color);
//                            $('.all_product').css('font-weight','bold');
////                           self.activeButton(ev);
//                        });
//                    });
//                    this._all_product();
                        }

            if(a_id == 'purchase.product_normal_action_puchased')
            {
            console.log("############################################ uuuuuuuuuuuuuuuuuuu", a_id);
                var total = this.$buttons.find('.purchased_product');
                $(total).css('background-color','#fffff');
                $(total).css('color','#4c4c4c');
                $(total).css('font-weight','bold');
                $(total).css('background', btn_color);
            ...
            if(a_id == 'product.product_template_action')
            {
            console.log("############################################ uuuuuuuuuuuuuuuuuuu", a_id);
                var total = this.$buttons.find('.sold_product');
                $(total).css('background-color','#fffff');
                $(total).css('color','#4c4c4c');
                $(total).css('font-weight','bold');
                $(total).css('background', btn_color);
...
//    shivam code end
    ...
    renderSidebar: function ($node) {
        if (this.hasSidebar && !this.sidebar) {
            ...
            // Devashish 20 March 2020 | Code for hide delete button on list view
...
//            }
    ...
    // Devashish 26 Nov | Reload listview when we click on reload button on top
    _onReloadListview: function (ev) {
        console.log("Reload button is clicked ............................ 2019");
        ev.stopPropagation();
        var self = this;
   ...
   // Shivam 02/12/20 code for adding header button on product.template
      activeButton:function(ev) {
        var self= this;
        $(".all_button").removeClass("active_btn_top");
    },

   _all_product: function (ev) {
        ev.stopPropagation();
        var self = this;
   ...
   _sold_product: function (ev) {
        ev.stopPropagation();
        var self = this;

        require('web.rpc').query({
            model: 'product.template',
            method: 'sold_product_show',
            args: [[]]
        }).then(function (result) {
            self.do_action(result).done(function(){
                $('.all_product').css('background-color','#fffff');
                $('.all_product').css('color','#4c4c4c');
                $('.all_product').css('font-weight','normal');
                $('.sold_product').css('background',btn_color);
                $('.sold_product').css('font-weight','bold');
               self.activeButton(ev);
            });
        });
   ...
   _purchased_product: function (ev) {
        ev.stopPropagation();
        var self = this;

        require('web.rpc').query({
            model: 'product.template',
            method: 'purchased_product_show',
            args: [[]]
        }).then(function (result) {
            self.do_action(result).done(function(){
                $('.all_product').css('background-color','#fffff');
                $('.all_product').css('color','#4c4c4c');
                $('.all_product').css('font-weight','normal');
                $('.purchased_product').css('background',btn_color);
                $('.purchased_product').css('font-weight','bold');
               self.activeButton(ev);
            });
        });
